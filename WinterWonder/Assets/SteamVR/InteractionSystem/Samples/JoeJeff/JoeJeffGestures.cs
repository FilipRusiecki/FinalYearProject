...
namespace Valve.VR.InteractionSystem.Sample
{
    public class JoeJeffGestures : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            for (int handIndex = 0; handIndex < PlayerVRRR.instance.hands.Length; handIndex++)
            {
                if (PlayerVRRR.instance.hands[handIndex] != null)
                {
                    SteamVR_Behaviour_Skeleton skeleton = PlayerVRRR.instance.hands[handIndex].skeleton;
                    if (skeleton != null)
                    {
