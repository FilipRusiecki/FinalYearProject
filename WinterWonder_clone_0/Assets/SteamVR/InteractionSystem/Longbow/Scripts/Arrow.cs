...
namespace Valve.VR.InteractionSystem
{
	...
	public class Arrow : MonoBehaviour
	{
		...
		void Start()
		{
			Physics.IgnoreCollision( shaftRB.GetComponent<Collider>(), PlayerVRRR.instance.headCollider );
		}
		...
		public void ArrowReleased( float inputVelocity )
		{
			...
			{
				if ( hit.collider.gameObject != gameObject && hit.collider.gameObject != arrowHeadRB.gameObject && hit.collider != PlayerVRRR.instance.headCollider )
				{
					Destroy( gameObject );
					return;
				}
		...
		void OnCollisionEnter( Collision collision )
		{
			if ( inFlight )
			{
				...
				if (PlayerVRRR.instance && collision.collider == PlayerVRRR.instance.headCollider )
				{
					PlayerVRRR.instance.PlayerShotSelf();
				}
			}
		}
